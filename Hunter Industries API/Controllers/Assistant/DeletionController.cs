    ...
    public class DeletionController : ApiController
    {
        ...
        public async Task<IHttpActionResult> Patch([FromBody] DeletionModel request, [FromUri] AssistantFilterModel filters)
        {
            ...
            if (!_modelValidator.IsValid(request) || !_modelValidator.IsValid(filters, true))
            {
                ...
                {
                    await _auditHistoryService.LogRequest(HttpContext.Current.Request.UserHostAddress, AuditHistoryConverter.GetEndpointID("assistant/deletion"), AuditHistoryConverter.GetEndpointVersionID(AuditHistoryFunction.ExtractVersionFromRequest(Request)), AuditHistoryConverter.GetMethodID("PATCH"), AuditHistoryConverter.GetStatusID("BadRequest"),
                        username, applicationName, new string[] { filters.AssistantName, filters.AssistantId, request.Deletion.ToString() });
                }
            ...
            if (await _configService.AssistantExists(filters.AssistantName, filters.AssistantId))
            {
                ...
                if (await _deletionService.AssistantDeletionUpdated(filters.AssistantName, filters.AssistantId, bool.Parse(request.Deletion.ToString())))
                {
                    ...
                    {
                        await _changeService.LogChange(audit.Item2, "Deletion", deletionResponse.Deletion.ToString(), request.Deletion.ToString());
                    }
                ...
                await _auditHistoryService.LogRequest(HttpContext.Current.Request.UserHostAddress, AuditHistoryConverter.GetEndpointID("assistant/deletion"), AuditHistoryConverter.GetEndpointVersionID(AuditHistoryFunction.ExtractVersionFromRequest(Request)), AuditHistoryConverter.GetMethodID("PATCH"),
                        AuditHistoryConverter.GetStatusID("InternalServerError"), username, applicationName, new string[] { filters.AssistantName, filters.AssistantId, request.Deletion.ToString() });
